...
#### Function for scaling NOLHD To between -1 and 1

scale_NOLHD<- function(mat){
  n=nrow(mat)
  cent= mean(seq(1,n))
  scale_mat = (mat-cent)/(n-cent)
  return(scale_mat)
}
...
prob_conditon_CS_joint <- function(c, sigma_sq, n, kstar,q,lambda,B1, sign_vec){
  ...
  #sign_vec=as.matrix(sign_vec)
  
  one_vec_inc = rep(1,kstar)
 
  z = as.vector(sign_vec%*%one_vec_inc)
...
prob_CS_joint_mean_lambda <-function(c, sigma_sq, n, kstar,q,lambda_list,B1, sign_vec){
  # browser()
  return(mean(sapply(lambda_list, prob_conditon_CS_joint, c=c, sigma_sq=sigma_sq,n=n, kstar=kstar, q=q,B1=B1, sign_vec = sign_vec)))
}
...
eval_exact_design_with_CS <- function(X, c_opt, df_curve){
  n <- nrow(X)
  p <- ncol(X)
  ...
  C_off_diag <- C[lower.tri(C)]
  max_dev    <- max(abs(c_opt - C_off_diag))
  
  upper_tri_C <- C[upper.tri(C)]  # was C_mat[upper.tri(C_mat)]
  sum_psi     <- sum(sapply(upper_tri_C, new_criterion_one_c, df_curve = df_curve))
...
# ##########################################################
# 
evalfunc = function(des, c_opt, df_curve, frob_or_sum = "sum") {
  
  # --- Input validation ---
  if (!frob_or_sum %in% c("sum", "frob")) {
    stop(paste0(
      "Invalid value for `frob_or_sum`: '", frob_or_sum, "'.\n",
      "  Accepted options are:\n",
      "    'sum'  : evaluates using the sum of psi values\n",
      "    'frob' : evaluates using the Frobenius norm\n",
      "  Please update your `frob_or_sum` argument accordingly."
    ))
  }
  
  nrun   <- nrow(des)
  results <- eval_exact_design_with_CS(des, c_opt, df_curve)
  ...
  if (frob_or_sum == "sum") {
    if (is.na(results$sum_psi)) { return(c(0, -1000)) }
    else { return(c(results$m_V, results$sum_psi)) }
  }
}
...
coexch1 = function(des, c_opt, df_curve,nfactor, frob_or_sum = "sum") {
  
  cdesmat = des
  cbestvals = evalfunc(des, c_opt, df_curve, frob_or_sum)
  ...
  while(cchange)
  {
    ...
    for (i in 1L:length(cdesmat))
    {
      ...
      for (j in level)
      {
        ...
        cnewdesmat[i] = j
        cnewbestval = evalfunc(cnewdesmat, c_opt, df_curve, frob_or_sum)
...
coexch2 = function(level, nrun, nfactor, c_opt, df_curve, frob_or_sum) {
  
  des = matrix(sample(level, nrun*nfactor, replace = TRUE), ncol = nfactor)
  start_score = evalfunc(des, c_opt, df_curve, frob_or_sum)
  ...
  #   dcheck = qr(crossprod(model.matrix(~ (x1+x2+x3)^2+I(x1^2)+I(x2^2)+I(x3^2), desfr)))$rank}
  #
  temp = coexch1(des, c_opt, df_curve,nfactor, frob_or_sum)
  psold = psnew = temp[[2L]]
  pfold = pfnew = temp[[1L]]
  ...
  while(sum(ind) > 0)
  {
    ...
    for (i in 1L:nrow(pfdiff))
    {
      ...
      temp = coexch1(desmat, c_opt,df_curve,nfactor, frob_or_sum)
      for(j in 1L:nrow(temp[[1L]]))
      {
        temppf = compare(matrix(temp[[1L]][j,], ncol = 2L), temp[[2L]][,j*nfactor-nfactor+1L:nfactor], pfnew, psnew, nfactor)
        pfnew = temppf[[1L]]
...
coexchpf = function(level, nrun, nfactor, ns, c_opt, df_curve, frob_or_sum)
{
  
  start_matrix = matrix(rep(0, 2*ns), nrow= ns, ncol=2)
  
  temp = coexch2(level, nrun, nfactor, c_opt, df_curve, frob_or_sum)
  paretodes = temp[[2]]
  paretodescrit = temp[[1]]
  start_matrix[1,]= temp[[3]]
  for (icount in 2L:ns)
  {
    
    temp = coexch2(level, nrun, nfactor, c_opt, df_curve, frob_or_sum)
    start_matrix[icount, ]=temp[[3]]
  ...
  list(paretodescrit, paretodes, start_matrix)
}

###### Section for New C measure

prob_CS_joint_mean_lambda_sign <- function(c, sigma_sq, n, kstar, q, 
                                           lambda_list, B1, sign_mat) {
  # Average over each row (sign vector) of sign_mat
  mean(apply(sign_mat, 1, function(sign_vec) {
    prob_CS_joint_mean_lambda(c        = c, 
                              sigma_sq = sigma_sq, 
                              n        = n, 
                              kstar    = kstar, 
                              q        = q, 
...
#'
#' @examples
get_c_curve_plot <- function(n, p, k, lambda_list, B1, signs = "all_pos", 
                             num_c_for_grid = 200, sigma_sq = 1, cores = 1) {
  
  # --- Input validation ---
  if (!signs %in% c("all_pos", "all")) {
    stop(paste0(
      "Invalid value for `signs`: '", signs, "'.\n",
      "  Accepted options are:\n",
      "    'all_pos' : uses a single all-positive sign vector\n",
      "    'all'     : averages over all 2^k possible sign vectors\n",
      "  Please update your `signs` argument accordingly."
    ))
  }
  
  # --- Setup ---
  # adding 0 to the sequence
  dense_c_grid <- sort(unique(c(0, seq(-(1 / (p - 1)), 0.9999, length.out = num_c_for_grid))))
  
  # --- Cluster setup ---
  cl <- makeCluster(cores)
  on.exit(stopCluster(cl))  # ensures cluster is stopped even if function errors
  
  clusterExport(cl, varlist = ls(envir = .GlobalEnv), envir = .GlobalEnv)
  clusterCall(cl, function(pkgs) lapply(pkgs, library, character.only = TRUE),
              pkgs = .packages())
  
  # --- Sign matrix ---
  if (signs == "all_pos") {
    sign_mat  <- matrix(rep(1, k), nrow = 1, ncol = k, byrow = TRUE)
    y_label   <- latex2exp::TeX("$\\psi_{\\Lambda}")
  } else if (signs == "all") {
    sign_mat  <- gen_all_sign_vects(seq(1, k, 1))
    y_label   <- latex2exp::TeX("$\\psi^{\\pm}_{\\Lambda}")
  }
  
  # --- Parallelized sapply ---
  psi_int_curve <- pbsapply(dense_c_grid,
                            prob_CS_joint_mean_lambda_sign,
                            sigma_sq    = sigma_sq,
                            n           = n,
                            kstar       = k,
                            q           = p - k,
  ...
  # --- Build plot data ---
  df_plot  <- as.data.frame(cbind(dense_c_grid, psi_int_curve))
  df_plot2 <- as.data.frame(cbind(seq(-1, -.13, length.out = 100), rep(0, 100)))
  colnames(df_plot2) <- c("dense_c_grid", "psi_int_curve")
  df_plot  <- as.data.frame(rbind(df_plot, df_plot2))
  
  # --- Plot ---
  gg1 <- ggplot() +
    geom_line(data = df_plot, 
              aes(x = as.numeric(dense_c_grid), y = as.numeric(psi_int_curve)), 
              size = 1.2) +
  ...
  # Finding max c from the dense grid
  

  psi_opt= max(df_plot$psi_int_curve)
  
  max_index = which.max(df_plot$psi_int_curve) 
  
  c_opt = df_plot$dense_c_grid[max_index]
  return(list("df_curve" = df_plot, "c_curve_plot" = gg1, "psi_max"= psi_opt, 
              "c_opt"= c_opt))
}


# function to linearly interpolate psi_int for a c value between dense grid values
psi_curve_interpolate <- function(new_c, df_curve){
  if(!is.data.frame(df_curve)) stop("df_curve is not a data frame, got: ", class(df_curve))
  
  # Handle NA correlation values
  if(is.na(new_c)) return(0)
  
  if(new_c < min(df_curve$dense_c_grid) | new_c > max(df_curve$dense_c_grid)){
    new_prob_int <- 0
  } else {
    new_prob_int <- approx(x = df_curve$dense_c_grid, y = df_curve$psi_int_curve, xout = new_c)$y
  }
  return(new_prob_int)
}

# Criteria for of correlation, for one off-diagonal for the exact C matrix
new_criterion_one_c <- function(new_c, df_curve){
  new_psi <- psi_curve_interpolate(new_c, df_curve)
...
# Applies criterion to exact design correlation matrix, C_mat. 
# Applied by summing single c criterion for upper triangular elements

new_criterion_C_matrix <- function(C_mat, df_curve){
  upper_tri_C = C_mat[upper.tri(C_mat)]
  
  crit <- sum(sapply(upper_tri_C, new_criterion_one_c,
                     df_curve=df_curve))
  return(crit)
}


# Applies criterion to exact design matrix itself. 
# Applied by summing single c criterion for upper triangular elements
...
new_criterion_X_matrix <- function(X, df_curve){
  ...
  p =ncol(X)
  X_cs = as.data.frame(lapply(as.data.frame(X), cent_scale))
  C_mat = t(as.matrix(X_cs))%*%as.matrix(X_cs)/n
  upper_tri_C = C_mat[upper.tri(C_mat)]
  crit <- sum(sapply(upper_tri_C, new_criterion_one_c,
                     df_curve=df_curve))
  return(crit)
...
# Create update function that takes in C_old and C_new and updates criterion





evaluate_designs_new_approach <- function( design_list, df_curve, c_opt){
  results_df <- do.call(rbind, lapply(names(design_list), function(design_name) {
    ...
    data.frame(
      design = design_name,
      tr_v = output_b$m_V,
      frob_norm = output_b$frob_norm,
      sum_c = output_a
    )
  }))
}

get_correl<- function(X){
  n= nrow(X)
  p =ncol(X)
  X_cs = as.data.frame(lapply(as.data.frame(X), cent_scale))
  C_mat= t(as.matrix(X_cs))%*%as.matrix(X_cs)/n
...
##### Code to do the EPCEA in one function step with an option to do the frob norm
#### or the sum_psi as a criterion with the tr(v).
...
#'
#' @examples
EPCEA_wrapper <- function(n, p, k, lambda_list, B1,level,ns, signs = "all_pos", 
                             num_c_for_grid = 200, sigma_sq = 1, cores = 1,
                          frob_or_sum = "sum") {
  ...
  # Get the df_curve output for the sum measure
  output <- get_c_curve_plot(n, p, k, lambda_list, B1, signs, 
                             num_c_for_grid, sigma_sq, cores)
  
  df_curve <- output$df_curve
  c_opt <- output$c_opt
  
  # Do the EPCEA for the pareto criteria
  
  EPCEA_results <- coexchpf(level, nrun=n, nfactor=p, ns, c_opt, df_curve,frob_or_sum)
  
  num_pf_designs = dim(EPCEA_results[2][[1]])[2]/p
  
  candidate_designs = EPCEA_results[2][[1]][,1:p]
  
  candidate_designs_list = list(EPCEA_results[2][[1]][,1:p])
  for (i in 2L:num_pf_designs){
    ind_1 =( (p*(i-1))+1)
    ind_2 = p*i
    candidate_designs=abind(candidate_designs,EPCEA_results[2][[1]][,ind_1: ind_2], along=3)
    candidate_designs_list[[length(candidate_designs_list)+1]]= EPCEA_results[2][[1]][,ind_1: ind_2]
    
  }
  
  df_pf= data.frame(EPCEA_results[[1]])
  colnames(df_pf) <- c("mean(tr(V))", "sum_psi")
  
  return( list("df_curve" = df_plot, 
               "c_curve_plot" = gg1, 
               "psi_max"= psi_opt, 
                    "c_opt"= c_opt, 
               "pareto_front_designs_list" = candidate_designs_list, 
               "pareto_front_designs" = candidate_designs,
               "pareto_front_results" = df_pf,
               "EPCEA_results"= EPCEA_results))
  
}
  
  
  eval_candidate_designs <- function(candidate_designs_list,candidate_designs,
                                     des.name.list, 
                                     n, p, k, 
                                     lambda_list, B1, df_curve,signs = "all_pos",
                                     sigma_sq = 1, cores = 1 ){
    
    
    names(candidate_designs_list) <- des.name.list
    k_star_list=c(k)
    B_mag_list = c(B1)
    ...
    grid<-expand.grid(k_star_list,B_mag_list)
    ...
    cl <- makeCluster(cores)
    registerDoParallel(cl)
    prob_results = foreach(start=1:length(candidate_designs_list), .combine = rbind, .packages =c('MASS',"mvtnorm","dplyr"),.export = ls(globalenv())) %dopar%{
      F_0 = candidate_designs[,,start]
      V_0_half= diag(lapply(as.data.frame(F_0),fn))
      F_0_cs = as.data.frame(lapply(as.data.frame(F_0), cent_scale))
      #log_lam = seq(-4.5, 2, by = 0.05)
      lambda = lambda_list
      joint_results_df = data.frame(design=character(0),k_star=numeric(0), B=numeric(0), lambda=numeric(0), P_joint=numeric(0))
      des.name=des.name.list[start]
      
      joint_results_df$design = factor(joint_results_df$design, levels=des.name.list)
      for (j in c(1:nrow(grid))){
        k_star=grid$Var1[j]
        B_cent=grid$Var2[j] 
        
        # all_signs = gen_all_sign_vects(seq(1,k_star,1))
        # submodels_all= gen_all_submodels(p=k,k_star)
        ...
        for (i in c(1:length(lambda))){
          ...
          prob_joint <-  measure_list(Joint_prob_general(F_cs =F_0_cs,V_0_half, 
                                                         B_mag=rep(B_cent,k_star),k=k_star,
                                                         sign_vects=sign_mat,
                                                         log_lambda = log(lam),
                                                         submodels = NULL,
          ...
          results_joint <- c(design=des.name, k_star=k_star,B=B_cent,lambda = lam, P_joint=prob_joint)
          index = dim(joint_results_df)[1]+1
          
          joint_results_df[index,]= results_joint
        }
      }
  ...
  generate_names = function(prefix, length) {
    paste(prefix, sprintf('%01d', seq_len(length)), sep = '_')
  }
